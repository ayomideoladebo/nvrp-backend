...
app.get('/api/economy/all-stats', async (req, res) => {
    ...
    try {
        const playerStats = await safeQuery("SELECT COUNT(*) FROM users as totalPlayers, (SELECT COUNT(*) FROM users WHERE is_online = 1) as playersToday FROM users");
        ...
        const topForklifters = await safeQuery("SELECT SUBSTRING_INDEX(description, ' got paid', 1) as player, SUM(CAST(REGEXP_SUBSTR(description, '[0-9]+') AS UNSIGNED)) as total FROM log_jobs WHERE description LIKE '%FOLKLIFTING%' GROUP BY player ORDER BY total DESC LIMIT 5");
