...
app.get('/api/player-route/:username', async (req, res) => {
    ...
    try {
        ...
        const [route] = await sampDbPool.query(
            "SELECT pos_x, pos_y, timestamp FROM player_locations WHERE player_id = ? AND timestamp > NOW() - INTERVAL 30 MINUTE ORDER BY timestamp ASC",
            [playerId]
        );
...
app.post('/api/gemini-analysis', async (req, res) => {
    ...
    if (previousCirculation === 0) {
        if (latestCirculation > 0) {
            ...
            circulationChangePercent = 22; // Representing a massive, uncapped positive change
